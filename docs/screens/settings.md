...
- Isolated relay `craysboardqa-venue-<run>` with: venue profile `30078` (`d=nuts-community-profile`, `type=hospitality`, admin-signed), a monthly sellable membership definition `30009` (`d=qa-membership-<run>`, `type=membership`, `t=membership`, `t=sellable`, `period=monthly`, `availability=available`, admin-signed), and a signed room manifest `30078` (`d=life.crays/room/v1/qa-room-<run>`, `schema=life.crays/room/v1`, `operator=<admin>`, capabilities, `open=open`, future expiration, `award_issuer=<badge issuer>`). The shared orders fixture family (product + issuer-signed award) is present but untouched by this flow.
...
- The venue relay retains exactly one `30009` at the membership `d`: `availability=unavailable`, classification/price/period retained, signed by the admin — a new event id at the same stable `d`.
- Read-only proof: the room manifest is byte-identical to the seeded event; the relay holds exactly two `30078` (profile + manifest) and two `30009` (product + membership) and zero `37237`.
